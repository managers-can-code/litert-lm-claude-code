...
def resolve_model_id(
    host: str, port: int, *, retries: int = 3, retry_delay: float = 0.4
) -> str | None:
    """Ask the server which model id it's serving.

    Retries transient connection errors (e.g. brief network blips when
    the MCP subprocess starts under load). A single timeout would
    otherwise cause the subagent to bail with "server not running" even
    ...
    for attempt in range(max(1, retries)):
        ...
        return None
    # Best-effort: log the last error so a follow-up status call can surface it.
    if last_err is not None:
        try:
            ensure_state_dir()
            (STATE_DIR / "last-resolve-error.txt").write_text(
...
def port_open(host: str, port: int, *, timeout: float = 1.0) -> bool:
    """Cheap TCP connectability check, independent of the HTTP layer."""
    try:
...
def last_resolve_error() -> str | None:
    """Return the last urllib error from resolve_model_id, if any."""
    p = STATE_DIR / "last-resolve-error.txt"
    if p.is_file():
        try:
