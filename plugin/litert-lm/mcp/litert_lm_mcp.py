...
# Optional debug logging. Off by default; set LITERT_LM_MCP_DEBUG=1 in the
# environment (or in the plugin.json mcpServers env block) to capture the
# JSON-RPC wire trace at /tmp/litert-lm-mcp-debug.log. Useful when MCP tool
# calls aren't reaching the handler and you need to know whether Claude Code
# even sent a tools/call.
_DEBUG_ENABLED = bool(os.environ.get("LITERT_LM_MCP_DEBUG"))
_DEBUG_LOG = "/tmp/litert-lm-mcp-debug.log"
...
def _dbg(msg: str) -> None:
    ...
    try:
        with open(_DEBUG_LOG, "a") as _f:
            _f.write(f"[{os.getpid()}] {msg}\n")
    except Exception:
        pass


_dbg(f"MODULE LOADED file={__file__}")
...
def tool_litert_lm_status() -> dict[str, Any]:
    ...
    Now includes ``port_open`` (cheap TCP connect probe) and ``http_error``
    (last urllib failure when /v1/models couldn't be parsed). When
    ``reachable`` is False but ``port_open`` is True, the server is alive
    but the HTTP probe transiently failed — usually a parsing/timeout
    issue rather than a "server not running" condition. Subagents should
    surface that distinction to the user instead of silently bailing.
    """
    ...
    is_port_open = CONTROL.port_open(host, port) if hasattr(CONTROL, "port_open") else None
    http_error: str | None = None
    if model_id is None and hasattr(CONTROL, "last_resolve_error"):
        http_error = CONTROL.last_resolve_error()
    return {
...
def tool_litert_lm_generate(
  ...
  """
  _dbg(f"ENTRY tool_litert_lm_generate prompt_len={len(prompt) if prompt else 0}")

  # No precondition status check — the POST itself is the probe. A
  # false-negative status check (e.g. transient timeout in the MCP
  # subprocess) would otherwise short-circuit a perfectly working
  # server. urllib's connection error gives us a precise reason if
  ...
  _dbg(f"BEGIN url={url}/v1/messages body_len={len(json.dumps(body))}")
  ...
  try:
    ...
    # Pre-probe at TCP level so we record what the OS thinks of the port
    # right before urllib runs.
    try:
      with _socket.create_connection((host, port), timeout=2.0):
        _dbg(f"TCP connect OK to {host}:{port}")
    except OSError as e:
      _dbg(f"TCP connect FAILED to {host}:{port}: {type(e).__name__}: {e}")
    ...
    _dbg(f"HTTP OK: response {len(raw)} bytes")
  except urllib.error.HTTPError as exc:
    _dbg(f"HTTPError: code={exc.code} reason={exc.reason}")
    return {"error": f"HTTP {exc.code}: {exc.reason}", "body": exc.read().decode("utf-8", errors="replace")}
  except urllib.error.URLError as exc:
    reason = getattr(exc, "reason", exc)
    _dbg(f"URLError: {type(reason).__name__}: {reason}")
    _dbg(f"  traceback: {_tb.format_exc()[:500]}")
    is_refused = "Connection refused" in str(reason) or "111" in str(reason)
    return {
        "error": (
            "litert-lm server is not running on "
            f"{url} (connection refused). Start it via /litert-lm-start."
            if is_refused
            else f"request failed: {reason}"
        ),
        "url": url,
        "reason": str(reason),
        "debug_log": _DEBUG_LOG if _DEBUG_ENABLED else None,
    }
  except Exception as exc:  # pylint: disable=broad-exception-caught
    _dbg(f"unexpected {type(exc).__name__}: {exc}\n{_tb.format_exc()[:500]}")
    return {"error": f"request failed: {exc}", "url": url, "debug_log": str(debug_log)}
...
def _run_official_sdk() -> int:
    ...
    def litert_lm_generate(
        ...
        Used by the litert-lm-local subagent for delegated inference.
        Returns the generated text plus token usage.
        """
        _dbg(f"SDK_GENERATE called prompt_len={len(prompt) if prompt else 0}")
        return tool_litert_lm_generate(
            prompt=prompt,
            model=model,
...
def _read_message() -> dict[str, Any] | None:
    ...
    if _DEBUG_ENABLED:
        try:
            _dbg(f"RECV method={json.loads(line).get('method', '?')} bytes={len(line)}")
        except Exception:
            pass
...
def _handle_tools_list(req_id: Any, _params: dict[str, Any]) -> dict[str, Any]:
    ...
    _dbg(f"tools/list -> {[t['name'] for t in tools]}")
...
def _handle_tools_call(req_id: Any, params: dict[str, Any]) -> dict[str, Any]:
    ...
    _dbg(f"tools/call name={name!r} arg_keys={list(arguments.keys())}")
    tool = _tool_by_name(str(name))
    if tool is None:
        _dbg(f"tools/call ERROR unknown_tool name={name!r} known_names={[t['name'] for t in TOOL_DEFINITIONS]}")
