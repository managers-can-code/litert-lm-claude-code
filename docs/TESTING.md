...
> **Known caveat for Mode 2 verification.** Some Claude Code builds close the MCP server's stdin immediately after `tools/list` (after discovery) and don't send `tools/call` to it for subagent-delegated tools — see upstream issues [anthropics/claude-code#37806](https://github.com/anthropics/claude-code/issues/37806) and [anthropics/claude-agent-sdk-python#597](https://github.com/anthropics/claude-agent-sdk-python/issues/597). When this happens, cloud Claude's subagent will *predict* a plausible response (e.g. answering "hi" to a "say hi" prompt) without the real local model running. To rule this out, enable the MCP debug log via the env var `LITERT_LM_MCP_DEBUG=1` (see *Verifying real local-model inference* below). If you only see `tools/list` followed by `STDIN EOF (server exiting)` and never `tools/call`, you're hitting this Claude Code limitation — your plugin is fine, the failure is upstream of it. The included `tier3-runner/launcher.py` + a manual JSON-RPC test (`outputs/verify-modes/diagnose-status-bug.command`) provide a direct end-to-end verification path that doesn't depend on the Claude Code MCP layer.
...
### Verifying real local-model inference

Cloud Claude is good at predicting plausible-looking responses. To prove the *actual* local model ran (rather than a Sonnet hallucination), enable the MCP debug log:

```bash
# Set in plugin.json's mcpServers.litert-lm.env block, OR run claude with it:
LITERT_LM_MCP_DEBUG=1 claude
```

After your delegation prompt, check the log:
...
```
[<pid>] MODULE LOADED file=...
[<pid>] PATH=fallback_stdio
[<pid>] RECV method=initialize
[<pid>] RECV method=notifications/initialized
[<pid>] RECV method=tools/list
[<pid>] tools/list -> [...]
[<pid>] RECV method=tools/call            ← key line: real invocation
[<pid>] tools/call name='litert_lm_generate' arg_keys=['prompt', ...]
[<pid>] ENTRY tool_litert_lm_generate prompt_len=NN
[<pid>] BEGIN url=http://127.0.0.1:9379/v1/messages body_len=NN
[<pid>] HTTP OK: response NNN bytes
```

If you only see up through `tools/list` followed by `STDIN EOF`, Claude Code never invoked the tool — see the caveat above. The launcher's stdout will also confirm: a real run shows a `POST /v1/messages` access-log line.

For an isolated, unambiguous test of the plugin itself (bypassing Claude Code entirely), see `outputs/verify-modes/diagnose-status-bug.command` — section E spawns the MCP via JSON-RPC directly and produces real Gemma output.
